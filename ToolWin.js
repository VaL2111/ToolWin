export class ToolWin {
	...
	createToolWin(nx, ny, buttonSize, images = [], tooltips = []) {
		...
		// Додавання елементів до ToolWin(вікна)
   ...
   createDefaultToolWinSize(nx, ny, buttonSize) {
		...
		const containerWidth = nx * buttonSize + (nx - 1) * 10; // Ширина кнопок + відступи між ними
		const containerHeight = ny * buttonSize + (ny - 1) * 10; // Висота кнопок + відступи між ними
		const toolWinTitleHeight = 50;
		const buttonContainerPadding = 10;
		const toolWinMinWidth = 100;

		// Задаємо розміри ToolWin
		toolwin.style.width = `${Math.max(toolWinMinWidth, containerWidth + buttonContainerPadding)}px`;
		toolwin.style.height = `${containerHeight + toolWinTitleHeight + buttonContainerPadding}px`;
	}
}